...
impl<Inner, U, R> Slots for Cons<Cons<Inner, U>, R>
where
    Cons<Inner, U>: Slots,
    R: Slots,
{
    ...
    fn all_slots() -> impl Iterator<Item = (TypeId, &'static str)> {
        Cons::<Inner, U>::all_slots().chain(R::all_slots())
    }
}

impl<R> Slots for Cons<Nil, R>
where
    R: Slots,
{
    fn try_slot<S>(self: Pin<&Self>) -> Option<Pin<&S>>
...
mod tests {
    ...
    fn cons_cons_nil_all_slots_returns_empty() {
        let slots = <Cons<Cons<Nil, Nil>, Nil>>::all_slots();
        assert_eq!(slots.count(), 0);
    }
