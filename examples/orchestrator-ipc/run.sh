...
# Run from the root of the repository, this is in `examples/orchestrator-ipc/` so two directories up.
...
in
  ping-pong)
    PING="$(cd examples/orchestrator-ipc && build ping)"
    PONG="$(cd examples/orchestrator-ipc && build pong)"
    TRACE="$(cd examples/orchestrator-ipc && build trace)"
  ;;

  useless)
    USELESS_MACHINE="$(cd examples/orchestrator-ipc && build useless-machine)"
  ;;
esac
...
in
  ping-pong)
    ...
    mod=examples_common::actors::ping_pong
