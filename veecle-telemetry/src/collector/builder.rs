...
/// Monotonic timestamp function (no epoch synchronization).
fn timestamp_fn_monotonic<T>() -> u64
where
    T: veecle_osal_api::time::TimeAbstraction,
{
    use veecle_osal_api::time::Instant;
...
/// System time timestamp function (with Unix epoch synchronization).
fn timestamp_fn_system_time<T>() -> u64
where
    T: veecle_osal_api::time::TimeAbstraction + veecle_osal_api::time::SystemTime,
{
    use veecle_osal_api::time::SystemTimeError;
...
impl<PID, EXP, TIME, THREAD> Builder<PID, EXP, TIME, THREAD> {
    /// Set the process ID for this collector instance.
    pub fn process_id(
        self,
    ...
    /// Set the exporter for telemetry data.
    pub fn exporter(
        self,
    ...
    /// Configure time abstraction (monotonic time only).
    ...
    /// Configure time abstraction with system time (Unix epoch synchronization).
    ...
    /// Configure thread abstraction.
    pub fn thread<Th>(self) -> Builder<PID, EXP, TIME, state::WithThread>
    where
        Th: veecle_osal_api::thread::ThreadAbstraction,
    {
        Builder {
            process_id: self.process_id,
            exporter: self.exporter,
...
impl Builder<state::WithProcessId, state::WithExporter, state::WithTime, state::WithThread> {
    /// Build this configuration into a [`Collector`] instance.
    ...
    /// Set this collector as the global collector.
    ///
