        ...
        match tracing_msg {
            TracingMessage::CreateSpan(span_msg) => {
                ...
                let parent_context = span_msg
                    .parent_span_id
                    .map(|parent_id| SpanContext::new(execution_id.process, parent_id));
            ...
            TracingMessage::AddEvent(event_msg) => {
                ...
                let span_context = SpanContext::new(execution_id.process, event_msg.span_id);
                let span = self
                    .spans
                    .get_mut(&span_context)
                    .expect("span should exist");
            ...
            TracingMessage::AddLink(link_msg) => {
                let span_context = SpanContext::new(execution_id.process, link_msg.span_id);
            ...
            TracingMessage::SetAttribute(attr_msg) => {
                let span_context = SpanContext::new(execution_id.process, attr_msg.span_id);
                let span = self
                    .spans
                    .get_mut(&span_context)
                    .expect("span should exist");
